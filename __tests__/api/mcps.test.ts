...
describe('MCPs API', () => {
  ...
  describe('POST /api/mcps', () => {
    it('should create a new MCP', async () => {
      ...
      mockStore.createMCP.mockImplementation((mcp: any) => {
        const created = {
          ...mcp,
          id: 'mcp-123',
        };
  ...
  describe('PUT /api/mcps/:id', () => {
    it('should update an existing MCP', async () => {
      ...
      mockStore.updateMCP.mockImplementation((id: string, updates: any) => {
        const mcp = mockStore.mcps.find((m: any) => m.id === id);
        if (mcp) {
  ...
  describe('DELETE /api/mcps/:id', () => {
    it('should delete an MCP', async () => {
      ...
      mockStore.mcps = [mcp];
      mockStore.deleteMCP.mockImplementation((id: string) => {
        mockStore.mcps = mockStore.mcps.filter((m: any) => m.id !== id);
      });
...
function setupMCPRoutes(app: express.Application, store: any) {
  app.get('/api/mcps', (_req, res) => {
    res.json(store.mcps);
  });

  app.get('/api/mcps/:id', (req, res) => {
    const mcp = store.mcps.find((m: any) => m.id === req.params.id);
    if (!mcp) {
