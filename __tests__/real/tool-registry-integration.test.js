...
describe('Tool Registry Integration - REAL', () => {
    ...
    describe('Tool Validation', () => {
        it('should validate tool parameters', () => {
            ...
            const pathParam = fileWriteTool?.params.find((p) => p.name === 'path');
            expect(pathParam).toBeTruthy();
            expect(pathParam?.required).toBe(true);
            const contentParam = fileWriteTool?.params.find((p) => p.name === 'content');
            expect(contentParam).toBeTruthy();
            expect(contentParam?.required).toBe(true);
        });
    });
});
//# sourceMappingURL=tool-registry-integration.test.js.map